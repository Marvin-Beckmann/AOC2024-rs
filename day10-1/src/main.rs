...
fn find_full_hiking_tracks(
    ...
    current_positions.insert((start_row, start_column));

    while current_height < 9 {
        current_height += 1;
        let mut new_current_positions = HashSet::new();
        for (x, y) in current_positions {
            ...
            for (nx, ny) in new_positions {
                if input[nx][ny] == current_height {
                    new_current_positions.insert((nx, ny));
                }
            }
        }
...
10456732";
    ...
    #[test]
    fn test() {
        assert_eq!(36, solve_1(TEST_1.to_string()));
    }
}
